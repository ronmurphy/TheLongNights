...
function parseEmoji(element) {
  ...
  // Skip elements with 'no-emoji-parse' class
  if (element.classList && element.classList.contains('no-emoji-parse')) {
    return;
  }
  ...
  const walker = document.createTreeWalker(
    ...
    {
      acceptNode: (node) => {
        // Skip nodes inside no-emoji-parse containers
        let parent = node.parentElement;
        while (parent) {
          if (parent.classList && parent.classList.contains('no-emoji-parse')) {
