...
class NebulaVoxelApp {
    constructor(container) {
        ...
        */
        ...
        // 🏰 HELPER: Determine player facing orientation for wall placement
        // Returns: 'north-south' or 'east-west' based on camera direction
        this.getPlayerOrientation = () => {
            // Get camera direction vector
            const direction = new THREE.Vector3();
            this.camera.getWorldDirection(direction);

            // Determine if player is facing more N/S or E/W
            // If looking along X axis (east/west), place wall N/S
            // If looking along Z axis (north/south), place wall E/W
            if (Math.abs(direction.x) > Math.abs(direction.z)) {
                return 'north-south'; // Looking E/W, so wall runs N/S
            } else {
                return 'east-west'; // Looking N/S, so wall runs E/W
            }
        };
        ...
        this.placeCraftedObject = (x, y, z, itemId) => {
            ...
            switch (shapeType) {
                ...
                case 'stairs':
                    ...
                    break;
                case 'defensive_wall':
                    // 🏰 DEFENSIVE WALL - Built with actual voxel blocks via StructureGenerator
                    console.log(`🏰 Building defensive wall ${dimensions.length}×${dimensions.height}×${dimensions.width}`);

                    // Determine orientation based on player facing direction
                    const wallOrientation = this.getPlayerOrientation();
                    console.log(`🧭 Wall orientation: ${wallOrientation}`);

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateWall(
                            x, z,
                            dimensions.length,  // Wall length
                            dimensions.height,  // Wall height
                            dimensions.width,   // Wall thickness (default 2)
                            wallOrientation,    // 'north-south' or 'east-west'
                            material,           // Wall material (stone)
                            this.addBlock.bind(this),
                            (wx, wz) => this.getGroundHeight(wx, wz)
                        );

                        this.updateStatus(`🏰 Built wall ${dimensions.length} blocks long!`, 'craft');
                        console.log(`✅ Defensive wall placed using ${material}`);
                        return; // Exit early - no mesh creation needed
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build wall - system error`, 'error');
                        return;
                    }
                    break;

                case 'defensive_tower':
                    // 🗼 DEFENSIVE TOWER - Built with actual voxel blocks via StructureGenerator
                    console.log(`🗼 Building defensive tower ${dimensions.length}×${dimensions.height}×${dimensions.width}`);

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateTower(
                            x, z,
                            dimensions.length,  // Tower base size (square)
                            dimensions.height,  // Tower height
                            material,           // Tower material (stone)
                            this.addBlock.bind(this),
                            (wx, wz) => this.getGroundHeight(wx, wz)
                        );

                        this.updateStatus(`🗼 Built tower ${dimensions.height} blocks tall!`, 'craft');
                        console.log(`✅ Defensive tower placed using ${material}`);
                        return; // Exit early - no mesh creation needed
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build tower - system error`, 'error');
                        return;
                    }
                    break;

                case 'castle_wall':
                    // 🏰 CASTLE WALL WITH TOWERS - Built with actual voxel blocks via StructureGenerator
                    console.log(`🏰 Building castle wall ${dimensions.length}×${dimensions.height} with towers`);

                    // Determine orientation based on player facing direction
                    const castleOrientation = this.getPlayerOrientation();
                    console.log(`🧭 Castle wall orientation: ${castleOrientation}`);

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateCastleWall(
                            x, z,
                            dimensions.length,  // Wall section length
                            dimensions.height,  // Wall height
                            castleOrientation,  // 'north-south' or 'east-west'
                            material,           // Wall material (stone)
                            this.addBlock.bind(this),
                            (wx, wz) => this.getGroundHeight(wx, wz)
                        );

                        this.updateStatus(`🏰 Built castle wall with towers!`, 'craft');
                        console.log(`✅ Castle wall placed using ${material}`);
                        return; // Exit early - no mesh creation needed
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build castle wall - system error`, 'error');
                        return;
                    }
                    break;

                case 'barricade':
                    // 🛡️ WOODEN BARRICADE - EARLY GAME
                    console.log(`🛡️ Building barricade ${dimensions.length}×${dimensions.height}`);

                    const barricadeOrientation = this.getPlayerOrientation();

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateBarricade(
                            x, z,
                            dimensions.length,
                            dimensions.height,
                        ...
                        this.updateStatus(`🛡️ Built wooden barricade!`, 'craft');
                        return;
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build barricade`, 'error');
                        return;
                    }
                    break;

                case 'spike_wall':
                    // 🔱 SPIKE WALL - EARLY GAME
                    console.log(`🔱 Building spike wall ${dimensions.length}×${dimensions.height}`);

                    const spikeOrientation = this.getPlayerOrientation();

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateSpikeWall(
                            x, z,
                            dimensions.length,
                            dimensions.height,
                            spikeOrientation,
                        ...
                        this.updateStatus(`🔱 Built spike wall!`, 'craft');
                        return;
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build spike wall`, 'error');
                        return;
                    }
                    break;

                case 'trench':
                    // 🚧 DEFENSIVE TRENCH - MID GAME
                    console.log(`🚧 Digging trench ${dimensions.length}×${dimensions.height}×${dimensions.width}`);

                    const trenchOrientation = this.getPlayerOrientation();

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateTrench(
                            x, z,
                            dimensions.length,
                            dimensions.height,  // Used as depth
                            dimensions.width,
                            trenchOrientation,
                            this.addBlock.bind(this),
                            (wx, wz) => this.getGroundHeight(wx, wz)
                        ...
                        this.updateStatus(`🚧 Dug defensive trench!`, 'craft');
                        return;
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not dig trench`, 'error');
                        return;
                    }
                    break;

                case 'archer_platform':
                    // 🏹 ARCHER PLATFORM - MID GAME
                    console.log(`🏹 Building archer platform ${dimensions.length}×${dimensions.height}`);

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateArcherPlatform(
                            x, z,
                            dimensions.length,  // Platform size (square)
                            dimensions.height,  // Platform height
                            material,
                            this.addBlock.bind(this),
                            (wx, wz) => this.getGroundHeight(wx, wz)
                        );

                        this.updateStatus(`🏹 Built archer platform!`, 'craft');
                        return;
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build archer platform`, 'error');
                        return;
                    }
                    break;

                case 'gatehouse':
                    // 🏰 GATEHOUSE - MID/LATE GAME
                    console.log(`🏰 Building gatehouse ${dimensions.length}×${dimensions.height}`);

                    const gatehouseOrientation = this.getPlayerOrientation();

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateGatehouse(
                            x, z,
                            dimensions.length,  // Gatehouse width
                            dimensions.height,  // Gatehouse height
                        ...
                        this.updateStatus(`🏰 Built gatehouse with murder holes!`, 'craft');
                        return;
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build gatehouse`, 'error');
                        return;
                    }
                    break;

                case 'bunker':
                    // ⚔️ UNDERGROUND BUNKER - LATE GAME
                    console.log(`⚔️ Building bunker ${dimensions.length}×${dimensions.height}`);

                    if (this.biomeWorldGen && this.biomeWorldGen.structureGenerator) {
                        this.biomeWorldGen.structureGenerator.generateBunker(
                            x, z,
                            dimensions.length,  // Bunker size (square)
                            dimensions.height,  // Bunker depth
                            material,
                            this.addBlock.bind(this),
                            (wx, wz) => this.getGroundHeight(wx, wz)
                        );

                        this.updateStatus(`⚔️ Built underground bunker!`, 'craft');
                        return;
                    } else {
                        console.error('❌ StructureGenerator not available');
                        this.updateStatus(`⚠️ Could not build bunker`, 'error');
                        return;
                    }
                    break;
        ...
        this.harvestWorldItem = (target) => {
            ...
            if (itemType === 'backpack' && !this.hasBackpack) {
                ...
                // Create companion portrait after backpack found
                if (this.companionPortrait) {
                    this.companionPortrait.create();
                }
            }
        ...
        this.getShapeIcon = (shape) => {
            const shapeIcons = {
                ...
                defensive_wall: 'fence',       // 🏰 Defensive wall icon
                defensive_tower: 'tower',      // 🗼 Tower icon
                castle_wall: 'castle',         // 🏰 Castle icon
                simple_house: 'home',          // 🏠 House icon
                barricade: 'fence',            // 🛡️ Barricade icon
                spike_wall: 'fence',           // 🔱 Spike wall icon
                trench: 'terrain',             // 🚧 Trench icon
                archer_platform: 'location_on', // 🏹 Platform icon
                gatehouse: 'castle',           // 🏰 Gatehouse icon
                bunker: 'layers'               // ⚔️ Bunker icon
            };
        ...
        this.getItemIcon = (itemType, context = 'inventory') => {
            ...
            const icons = {
                ...
                defensive_wall: '🧱',          // Defensive wall with battlements
                defensive_tower: '🗼',         // Multi-level tower
                castle_wall: '🏰',             // Castle wall with towers
                barricade: '🛡️',               // Wooden barricade
                spike_wall: '🔱',              // Spike wall
                trench: '🚧',                  // Defensive trench
                archer_platform: '🏹',         // Archer platform
                gatehouse: '🏰',               // Gatehouse
                bunker: '⚔️',                  // Underground bunker
        ...
        const mousedownHandler = (e) => {
            ...
            if (intersects.length > 0) {
                ...
                if (e.button === 0) { // Left click - harvesting (blocks or crafted objects)
                    ...
                    // 🏹 CHECK RANGED WEAPONS FIRST: These work on enemies without needing hit.face
                    ...
                    if (isRangedWeapon && selectedSlot.quantity > 0) {
                        // Fire ranged weapon at the target position (works on enemies, blocks, etc.)
                        this.craftedTools.fireRangedWeapon(selectedBlock, pos);
                        return; // Don't continue to block placement
                    }

                    // Safety check: hit.face can be null when clicking on sky/void or sprites
                    // Only needed for block placement and other tools
                    if (!hit.face) {
